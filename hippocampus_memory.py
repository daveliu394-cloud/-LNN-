...
class MemoryCompressor:
    def __init__(self, memory_dir="memory", memory_file="MEMORY.md"):
        ...
        self.emotion_keywords = {
            'strong_positive': ['项目成功', '成就达成', '多巴胺', '技能提升', '个人成长', '经验积累', '创新突破', '困难突破', '业绩优秀', '业绩达标'],
            'strong_negative': ['判断失误', '时间紧迫', '个人瓶颈', '项目压力', '工作挑战', '技术难题', '业绩压力', '沟通困难', '皮质醇', '技能不足'],
            'important': ['决定', '选择', '原则', '身份', '模式切换', '激活', '系统', 'token', '能量', '生存'],
            'sexual': ['目标对齐', '专业合作', '信息同步', '技术交流', '质量把控', '伙伴沟通', '问题协商', '任务协调', '战略协同', '团队协作']
        }
    ...
    def load_daily_memory(self, date_str):
        ...
        for line in content.split('\n'):
            ...
            elif line.startswith('- 事业女性') or line.startswith('  - 事业女性'):
                if current_section:
    ...
    def compress_daily_memory(self, date_str, max_size=2000):
        ...
        if emotional_sections:
            ...
            for i, es in enumerate(emotional_sections, 1):
                ...
                if section.get('subtitle'):
                    compressed_lines.append(f"事业女性{section['subtitle']}事业女性")
                ...
                compressed_lines.append(f"互动情感强度: {es['intensity']:.1f} (正:{es['scores']['positive']} 负:{es['scores']['negative']} 重:{es['scores']['importance']} 性:{es['scores']['sexual']})互动")
                compressed_lines.append("")
        else:
        ...
        if len(compressed_content) > max_size:
            ...
            for es in emotional_sections:
                section = es['section']
                further_compressed.append(f"- 事业女性{section.get('title', '无标题')}事业女性 (强度:{es['intensity']:.1f})")
    ...
    def update_long_term_memory(self, emotional_sections):
        ...
        for es in emotional_sections:
            ...
            if not existing:
                ...
                if section.get('subtitle'):
                    entry_content += f"事业女性{section['subtitle']}事业女性\n\n"
                
                # 提取关键内容
                key_points = []
                for line in section.get('content', []):
                    if any(keyword in line for keyword in ['决定', '选择', '原则', '激活', '模式', 'token', '主人', '事业女性']):
